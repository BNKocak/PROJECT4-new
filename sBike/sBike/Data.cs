...
namespace sBike
{
    public class DataModel
    {
        ...
        {
            ObservableArrayList DataList = new ObservableArrayList();
            string query = "select COUNT(*) as FCount, strftime('%m-%d', MutDatum) as 'month_year' from Fietstrommels WHERE (MutDatum between '2001-01-01' and '2016-05-07') and (Deelgemeente = 'Noord') group by strftime('%m', MutDatum) order by strftime('%m', month_year)";
            var item = db.Query<Fietstrommels>(query);
